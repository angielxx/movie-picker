<template>
    <div class="UserItem">
        ...
        :style="`background-image: url(${this.$store.state.API_URL}${this.user.user.avatar})`"
        @click="goUserDetail"
        >
        
        </div>
        <h2>{{ this.user.user.username }}</h2>
...
export default {
    ...
    computed: {
        checkFollow() {
            if (this.$store.state.followings.includes(this.user.id)) return true
            return false
        }
    },
