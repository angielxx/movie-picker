<template>
  <div class="search-bar">
    ...
    <form action="" class="search-bar__form">
      <input type="text" :placeholder="placeholder" @click="goSearch" @focus="goSearch" @input="searchMovie($event)">
...
export default {
  ...
  computed: {
    placeholder() {
      let placeholder;
      if (['feed', 'userSearch'].includes(this.$route.name)) {
        placeholder = '유저 바로 검색하기'
      } else {
        placeholder = '영화 바로 검색하기'
      }
      return placeholder
    }
  },
  ...
  methods: {
    ...
    goSearch() {
      // route에 따라 클릭시 이동
      if (['home', 'movieDetail'].includes(this.$route.name)) {
        this.$router.push({ name: 'search'})
      } else if(['feed'].includes(this.$route.name)) {
